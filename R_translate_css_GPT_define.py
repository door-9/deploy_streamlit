...
def create_two_subplots(x_cl, y_cl, b_cl, log_a_cl, 
    ...
    axes[1].set_ylabel('Log10(Vd(L/kg))')
...
def calc_sa(animal_df, human_df):
    ...
    - 사람: CL_h= (a*(BW_h^b))/BW_h
            Vd_h= (a_vd*(BW_h^b_vd))/BW_h
    """
    dfA = animal_df.dropna(subset=["BW(kg)","CL(L/hr/kg)","Vd(L/kg)"])
    dfA = dfA[(dfA["BW(kg)"]>0)&(dfA["CL(L/hr/kg)"]>0)&(dfA["Vd(L/kg)"]>0)]
    if len(dfA)<2:
        return None

    # CL part
    x_cl= np.log10(dfA["BW(kg)"].values)
    y_cl= np.log10(dfA["CL(L/hr/kg)"].values)
    b_cl, log_a_cl= np.polyfit(x_cl, y_cl, 1)
    a_cl= 10**log_a_cl

    # Vd part
    x_vd= np.log10(dfA["BW(kg)"].values)
    y_vd= np.log10(dfA["Vd(L/kg)"].values)
...
def calc_mlp(animal_df, human_df):
    ...
    - 사람: CL_h= (a_roe*(BW_h^b_roe)) / (BW_h* MLP_h)
    - Vd => SA
    """
    dfA= animal_df.dropna(subset=["BW(kg)","CL(L/hr/kg)","Vd(L/kg)","MLP(year)"])
    dfA= dfA[(dfA["BW(kg)"]>0)&(dfA["CL(L/hr/kg)"]>0)&(dfA["Vd(L/kg)"]>0)&(dfA["MLP(year)"]>0)]
    if len(dfA)<2:
        return None
...
def calc_brw(animal_df, human_df):
    ...
    - 사람: CL_h= (a_brw*(BW_h^b_brw)) / (BW_h * BrW_h)
    - Vd => SA
    """
    dfA= animal_df.dropna(subset=["BW(kg)","CL(L/hr/kg)","Vd(L/kg)","BrW(kg)"])
    dfA= dfA[(dfA["BW(kg)"]>0)&(dfA["CL(L/hr/kg)"]>0)&(dfA["Vd(L/kg)"]>0)&(dfA["BrW(kg)"]>0)]
    if len(dfA)<2:
        return None

    x_cl= np.log10(dfA["BW(kg)"].values)
    cl_times_brw= dfA["CL(L/hr/kg)"].values * dfA["BrW(kg)"].values
    ...
    CL_h= (a_brw*(BW_h**b_brw)) / (BW_h*BrW_h)
    Vd_h= (a_vd*(BW_h**b_vd))/BW_h
    t12= calc_half_life(Vd_h, CL_h)
...
def main():
    ...
    if st.button("Calculate"):
        dfA = animal_df.copy()
        for c in ["BW(kg)","CL(L/hr/kg)","Vd(L/kg)","MLP(year)","BrW(kg)"]:
            if c in dfA.columns:
        ...
        if method=="Single species":
            ...
            cl_an= float(rowA["CL(L/hr/kg)"]) if rowA["CL(L/hr/kg)"]>0 else 0.0
            vd_an= float(rowA["Vd(L/kg)"]) if rowA["Vd(L/kg)"]>0 else 0.0
